...
VibePromptRig ships as the `vibepromptrig` Claude Code plugin. The 5.1.7 package has 14 agents, 9 skills, 3 commands, 4 workflows, the `forge` tools, and a compact Context V2 rule resolver. The installable boundary is the generated `plugin-dist/` directory.
